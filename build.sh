...
# build gateway
cd cmd/gateway
GOARCH=amd64 GOOS=linux go build -ldflags "-X 'main.commit=$COMMIT_TAG' -X 'main.date=$BUILT_AT'" -o short_url_gateway .
...
cd cmd/server
GOARCH=amd64 GOOS=linux go build -ldflags "-X 'main.commit=$COMMIT_TAG' -X 'main.date=$BUILT_AT'" -o short_url_server .
mv short_url_server ../..
cd -

cd cmd/conf
GOARCH=amd64 GOOS=linux go build -ldflags "-X 'main.commit=$COMMIT_TAG' -X 'main.date=$BUILT_AT'" -o short_url_conf .
mv short_url_conf ../..
cd -
