...
func DeferWhenCoreDump() {
	if err := recover(); err != nil {
		//打印栈信息
		stack := debug.Stack()
		logger.MainLogger.Error(string(stack))
		panic(err)
	}
}

func GenCredentials(caFile, pemFile, keyFile, serverName string) (serverCreds, clientCreds credentials.TransportCredentials, err error) {
	cert, _ := tls.LoadX509KeyPair(pemFile, keyFile)
	...
	clientCreds = credentials.NewTLS(&tls.Config{
		...
		ServerName:         serverName,
		RootCAs:            certPool,
		InsecureSkipVerify: true, //@TODO 此处是否不检查？
	})
	return
}

func MatchReg(regStr, targetStr string) (match bool, err error) {
	match, err = regexp.Match(regStr, []byte(targetStr))
	return
}



//Elapsed calc how long pass from start
//return ms
