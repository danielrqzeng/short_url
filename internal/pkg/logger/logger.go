...
func InitLogger(logDir string, logLever zapcore.Level) {
	...
	GrpcLogger = NewLogger(path.Join(logDir, "/grpc.log"), zapcore.InfoLevel, 128, 30, 7, true, "grpc")
...
func InitOneLogger(loggerName, fileName string, logLever zapcore.Level) {
	switch loggerName {
	case "main":
		MainLogger = NewLogger(fileName, logLever, 32, 30, 7, true, "Main")
	case "http":
		HttpLogger = NewLogger(fileName, logLever, 32, 30, 7, true, "http")
	case "grpc":
		GrpcLogger = NewLogger(fileName, logLever, 32, 30, 7, true, "grpc")
	case "api":
		APILogger = NewLogger(fileName, logLever, 32, 30, 7, true, "api")
	case "db":
		DBLogger = NewLogger(fileName, logLever, 32, 30, 7, true, "db")
	}
}
